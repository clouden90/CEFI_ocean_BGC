...
!            implementation='duce',   &
  ...
  !     ilb,jlb,tau,dt,grid_dat,model_time,nbands,max_wavelength_band,sw_pen_band,opacity_band,internal_heat,frunoff)
    ...
    real :: sfc_irr_loc, irr_band_loc(nbands), pcmlim_ML(NUM_PHYTO)  ! GPU §1.2b-A per-thread scratch
    integer :: kbl
...
!
    ...
    enddo
    ! === GPU §2 MERGE step-4 (FINAL): growth folded in -> ONE resident region for the WHOLE foodweb
    !     (growth->bacteria->zoo->losses->production, all 17 kernels, one enter / one exit). Growth's §1.2b enter
    !     ABOVE keeps its correct to:/alloc categorization (the §1.3 alloc-vs-to: fix); we APPEND only the
    !     downstream-section arrays growth does not map. Growth's phyto outputs (juptake_*, q_*, f_mu_mem, P_C_max,
    !     nh4lim, jdissloss_si, jexuloss_fe) + expkT are ALREADY resident -> downstream stops re-loading them (the
    !     big HtoD win). f_nh3 alloc here; host nh3-island (L3749) writes it; target update to(f_nh3) bridges it. ===
    !$omp target enter data map(to: zoo, bact, hblt_depth)
    !$omp target enter data map(to: cobalt%f_ndet,cobalt%f_ndet_fast,cobalt%f_pdet,cobalt%f_pdet_fast, &
    !$omp&   cobalt%f_fedet,cobalt%f_sidet,cobalt%zt,cobalt%hp_jingest_fe,cobalt%hp_jingest_sio2, &
    !$omp&   cobalt%f_ldon,cobalt%f_ldop)
    !$omp target enter data map(to: cobalt%jprod_ndet,cobalt%jprod_pdet,cobalt%jprod_sldon,cobalt%jprod_srdon, &
    !$omp&   cobalt%jprod_sldop,cobalt%jprod_srdop,cobalt%jprod_fedet,cobalt%jprod_sidet,cobalt%jprod_ndet_fast, &
    !$omp&   cobalt%jprod_pdet_fast,cobalt%jprod_fed,cobalt%jprod_sio4,cobalt%jno3denit_wc)
    !$omp target enter data map(alloc: cobalt%hp_temp_lim,cobalt%hp_o2lim,cobalt%hp_jingest_n,cobalt%hp_jingest_p, &
    !$omp&   cobalt%total_filter_feeding,cobalt%juptake_nh4amx,cobalt%juptake_no3amx,cobalt%jnamx, &
    !$omp&   cobalt%juptake_nh4nitrif,cobalt%jprod_no3nitrif,cobalt%f_nh3)
    do n = 1,NUM_PHYTO
      !$omp target enter data map(alloc: phyto(n)%jzloss_n,phyto(n)%jzloss_p,phyto(n)%jzloss_fe,phyto(n)%jzloss_sio2, &
      !$omp&   phyto(n)%stress_fac,phyto(n)%jaggloss_n,phyto(n)%jaggloss_p,phyto(n)%jaggloss_fe,phyto(n)%jaggloss_sio2, &
      !$omp&   phyto(n)%jmortloss_n,phyto(n)%jmortloss_p,phyto(n)%jmortloss_fe,phyto(n)%vmove,phyto(n)%jvirloss_n, &
      !$omp&   phyto(n)%jvirloss_p,phyto(n)%jvirloss_fe,phyto(n)%jvirloss_sio2,phyto(n)%jexuloss_n,phyto(n)%jexuloss_p)
    enddo
    do m = 1,NUM_ZOO
      ...
      !$omp&   zoo(m)%jprod_ndet,zoo(m)%jprod_pdet,zoo(m)%jprod_sldon,zoo(m)%jprod_ldon,zoo(m)%jprod_srdon, &
      !$omp&   zoo(m)%jprod_sldop,zoo(m)%jprod_ldop,zoo(m)%jprod_srdop,zoo(m)%jprod_fedet,zoo(m)%jprod_sidet, &
      !$omp&   zoo(m)%jprod_n,zoo(m)%jprod_nh4,zoo(m)%jprod_po4,zoo(m)%jprod_fed,zoo(m)%jprod_sio4)
    enddo
    ...
    !
    ! === GPU §1.2b Loop A (irradiance/photoacclimation -- COLUMNAR): omp target teams loop collapse(2)
    !     over (j,i) with k sequential per thread. Restructured for GPU (single source; CPU still correct):
    !       - per-thread private scratch irr_band_loc(nbands) (was reused down each column -> would race),
    ...
    do j = jsc, jec ; do i = isc, iec   !{
       ...
       ! Calculate photosynthetically available radiation at air-sea interface (sfc_irr_loc)
       sfc_irr_loc = 0.0
       do nb=1,nbands !{
          if (max_wavelength_band(nb) .lt. 710.0) then !{
             irr_band_loc(nb) = cobalt%par_adj*sw_pen_band(nb,i,j)
             sfc_irr_loc = sfc_irr_loc + cobalt%par_adj*sw_pen_band(nb,i,j)
          else
             irr_band_loc(nb) = 0.0
          endif !}
       enddo !}

       ! day length (CBM model); rev_angle/dec_angle hoisted above (constant per call)
       ...
       cobalt%f_irr_aclm_sfc(i,j,1) = (cobalt%f_irr_aclm_sfc(i,j,1) + &
         (sfc_irr_loc*24.0/max(cobalt%daylength(i,j),cobalt%min_daylength)-cobalt%f_irr_aclm_sfc(i,j,1)) * &
         min(1.0,cobalt%gamma_irr_aclm * dt)) * grid_tmask(i,j,1)

       kbl = 0               ! per-thread max k index within the mixed layer
       tmp_irrad_ML = 0.0
       tmp_hblt = 0.0
       tmp_irrad_aclm = 0.0
       tmp_zaclm = 0.0
       do n = 1,NUM_PHYTO
         pcmlim_ML(n) = 0.0  ! per-thread accumulator (was phyto%tmp_pcmlim_aclm_ML)
       enddo
       ...
       do k = 1, nk !{
          ...
          do nb=1,nbands !{
             ...
             tmp_irrad = tmp_irrad + max(0.0,irr_band_loc(nb) * exp(-tmp_opacity * dzt(i,j,k) * 0.5))
             irr_band_loc(nb) = irr_band_loc(nb) * exp(-tmp_opacity * dzt(i,j,k))
          enddo !}
          ...
          if ( (k == 1) .or. (tmp_hblt .lt. cobalt%mld_aclm(i,j)) ) then
             ...
             do n = 1,NUM_PHYTO
                pcmlim_ML(n) = pcmlim_ML(n)+phyto(n)%pcmlim_aclm_inst(i,j,k)*dzt(i,j,k)
             enddo
       ...
       ! Surface mixed layer (1:kbl) acclimates to mean daytime light over the photoacclimation layer.
       ! (1:kblt) array-section writes -> explicit loop for the kernel.
       do k = 1, kbl
          ...
          do n = 1,NUM_PHYTO
            phyto(n)%pcmlim_aclm_inst(i,j,k) = pcmlim_ML(n) / max(1.0e-6,tmp_hblt)
          enddo
       enddo
    enddo;  enddo !} i,j
    ...
    ! === GPU §2 MERGE step-4 bridge: nh3 island writes f_nh3 on HOST; push to device for bacteria's nitrif kernel
    !     (downstream in the same resident region). Unconditional so the device copy is current either way. ===
    !$omp target update to(cobalt%f_nh3)
...
! === GPU §1.2b Loops E+F (ML growth-avg + mu_mem relax): ONE residency scope; mu_mix stays resident E->F.
    ...
    ! === GPU §2 MERGE step-4: growth's exit REMOVED — growth outputs stay DEVICE-RESIDENT through
    !     bacteria+zoo+losses+production; released at the single fused exit after production. ===
...
!
    ...
    ! === GPU §2 MERGE step-4: bacteria's enter REMOVED — the whole foodweb working set is already RESIDENT from
    !     the step-4 enter before growth. bacteria/zoo/losses/production kernels run directly on resident data. ===
    ...
    enddo; enddo ; enddo !} i,j,k
    ! === GPU §2 MERGE step-3: bacteria's exit REMOVED — bacteria outputs stay DEVICE-RESIDENT through
    !     zoo+losses+production; released at the single fused exit after production. ===
!
...
!
    ...
    enddo; enddo; enddo  !} i,j,k
    ! === GPU §2 MERGE step-2: zoo's exit REMOVED — zoo outputs stay DEVICE-RESIDENT through losses+production;
    !     they (and zoo's inputs) are released at the single fused exit after production. ===
    ...
    ! === GPU §2 MERGE step-2: losses+production enter REMOVED — the whole working set (losses inputs/outputs +
    !     production inputs) is already RESIDENT from the fused zoo+losses+production enter above. The losses
    !     kernels run directly on resident data; the vmove bridge + single fused exit (after production) follow. ===
    ...
    enddo; enddo ; enddo !} i,j,k
    ! === GPU §2 MERGE step-4 (FINAL): SINGLE fused exit for the WHOLE foodweb (growth->...->production). from:
    !     every output the still-CPU downstream (source/sink, diag) + next timestep need — growth (phyto q_*/
...
!
    ...
    ! === GPU §3 M2 (corrected, loop2-validation): foodweb+ballast exit RELOCATED to after loop2 (region stays open
    !     over loop1+loop2). Unlike the failed M2-a, loop2's per-loop foodweb-resident maps are STRIPPED (no double-
    !     mapping of cobalt/phyto whole types or resident members) — loop2 reads them resident. Bridge-1 added for
    !     loop1 net_src. Loops 3/5/6 remain own-region (exit closes the region before them). ===
...
!
    ...
    ! GPU §3 M2 bridge-1: net_src (CPU) reads these foodweb-produced arrays, now resident in the merged region (open
    ! over loop1+loop2) -> refresh host. (pre_tot reads p_* which are not device-mapped here -> host already valid.)
    !$omp target update from(cobalt%jnamx,cobalt%jno3denit_wc,phyto(DIAZO)%juptake_n2)
    do k = 1, nk ; do j = jsc, jec ; do i = isc, iec  !{
    ...
    ! GPU §3 M2: loop2 reads cobalt/phyto RESIDENT (foodweb region open) — NO per-loop map (this avoids the
    ! double-map of whole types + resident members that corrupted M2-a). Only source/sink-specific maps remain.
    ...
    ! === GPU §3 M2 (FULL): the foodweb+ballast exit is now relocated to AFTER loop6 (see below). The merged resident
    !     region spans foodweb -> ballast -> source/sink loops 1-6; the ~100 intermediates never DtoH here / never
    !     re-HtoD in source/sink. Loops 3/5/6 have their resident maps stripped (read resident). ===
    !
    ...
    ! GPU §3 M2: loop3 reads cobalt/phyto/bact RESIDENT (merged region open) — no per-loop map (avoids double-map).
    ...
    ! GPU §3 M2 bridge-2: loop4 (zoo, CPU island inside the merged region) reads zoo tendencies now resident -> refresh host
    do m = 1,NUM_ZOO
      !$omp target update from(zoo(m)%jprod_n,zoo(m)%jzloss_n,zoo(m)%jhploss_n)
    enddo
    do k = 1, nk ; do j = jsc, jec ; do i = isc, iec  !{
       !
       ! Small zooplankton
...
!
    ...
    ! GPU §3 M2: loop5 reads cobalt/phyto RESIDENT — strip resident maps; KEEP host inputs jfe_coast/jfe_iceberg
    ! (CPU bottom-sediment/iceberg islands, not resident). jno3_iceberg/jpo4_iceberg stay with the p_* directive.
    !$omp target enter data map(to: cobalt%jfe_coast,cobalt%jfe_iceberg)
    ...
    ! GPU §3 M2: release only the host inputs loop5 mapped (resident foodweb/ballast arrays freed at the merged exit).
    !$omp target exit data map(delete: cobalt%jfe_coast,cobalt%jfe_iceberg,cobalt%jno3_iceberg,cobalt%jpo4_iceberg)
    ...
    ! GPU §3 M2: loop6 reads cobalt/phyto/bact RESIDENT — strip resident maps; KEEP host jdic_caco3_nerbur (neritic
    ! burial CPU island output, not resident). jprod_fedet is foodweb-resident and is RMW'd in-kernel here, then
    ! DtoH'd at the merged exit — still resident, no per-loop map needed.
    !$omp target enter data map(to: cobalt%jdic_caco3_nerbur)
    ...
    !$omp&   cobalt%p_ldop,cobalt%p_sldon,cobalt%p_sldop,cobalt%p_srdon,cobalt%p_srdop,cobalt%p_o2,cobalt%p_alk, &
    !$omp&   cobalt%p_dic)
    ! GPU §3 M2: release only the host input loop6 mapped (jdic_caco3_nerbur). Resident arrays + jprod_fedet
    ! (RMW'd in-kernel, stays resident) are DtoH'd/freed at the merged exit, NOT here.
    !$omp target exit data map(delete: cobalt%jdic_caco3_nerbur)
!

    if (do_14c) then                                        !<<RADIOCARBON
      ! GPU §3 M2 bridge-3: this CPU radiocarbon block reads foodweb/ballast arrays still resident in the merged
      ! region -> refresh host. Gated by do_14c (OFF in the b2b config), so hand-verified, not b2b-covered.
      !$omp target update from(cobalt%f_ndet,cobalt%f_ndet_fast,cobalt%jremin_ndet,cobalt%jremin_ndet_fast, &
      !$omp&   cobalt%jprod_ndet,cobalt%jprod_ndet_fast,cobalt%jprod_srdon,cobalt%jprod_cadet_arag, &
      !$omp&   cobalt%jprod_cadet_calc,cobalt%jdiss_cadet_arag,cobalt%jdiss_cadet_calc,cobalt%jno3denit_wc, &
      !$omp&   phyto(DIAZO)%juptake_n2)
...
!
    ...
    ! === GPU §3 M2 (FULL): RELOCATED foodweb+ballast+source/sink exit — closes the ONE merged resident region here,
    !     after all GPU compute + the per-loop p_* writes + the do_14c host island. DtoH the intermediates + delete
    !     inputs once. Foodweb intermediates were resident the whole time (no re-HtoD in source/sink). ===
